...
★ 분량 절대 준수: compatibilityDetails 11개는 하나도 빠짐없이 각각 최소 600자·2~3문단이어야 한다. 출력이 길다고 뒤쪽 항목을 점점 짧게 압축하는 것을 절대 금지한다 — 1번과 11번의 밀도가 같아야 유료 상품이다)
...
JSON 포맷:\n
{
  ...
  "vsCards": [
    { "topic": "비교 주제 (예: 갈등 스타일)", "myTrait": "나의 성향 한 줄", "partnerTrait": "상대방 성향 한 줄", "explanation": "두 성향이 만나면 실제 연애에서 어떤 역학·충돌이 발생하는지 구체적으로 설명하되, 반드시 실제 대화 한 토막을 포함할 것 (예: 나영: \\"오늘 너무 힘들었어\\" / 혁준: \\"그럼 다음부턴 이렇게 해\\") — 대화 한 줄이 어떤 설명보다 체감이 크다. 가독성을 위해 2~3문단으로 나누고 문단 사이 줄바꿈 두 번(\\n\\n), 대화 토막은 자체 문단으로 분리할 것 (최소 300자)" },
    ... (정확히 4개)
  ],
