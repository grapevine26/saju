...
// ─────────────────────────────────────
// 운명의 합 — 무료 미리보기 전용 AI 티저 (결정론 점수만으로는 "내 얘기다"
// 싶은 순간이 없어 전환이 약하다 — 재회사주 lite(prompt1)의 secretTeaser
// 패턴을 궁합용으로 이식. 짧고, 진단만 하고, 처방은 절대 안 준다)
// ─────────────────────────────────────
...
위 데이터를 바탕으로 무료 미리보기용 짧은 진단과 결제 유도 티저를 작성해줘.

(★ 무료 미리보기 구간의 특별 규칙 — 분량이 길면 결제 동기가 떨어진다. 짧고 강하게:
1. essence는 300~400자·정확히 2문단. 두 사람의 궁합에서 가장 눈에 띄는 성향·궁합 패턴을 콕 짚어 "우리 얘기다" 싶게 만드는 데 집중하되, "~하는 편입니다" 같은 성향 언어로 써라. 오행·십성·합충 용어 노출 금지, 자연 비유(산·꽃·강·햇살 등)와 심리·관계 언어로만 번역.
2. 구체적 '처방'(무엇을 언제 어떻게 하라)은 절대 쓰지 마라 — 그것은 유료 리포트의 몫이다. 진단(어떤 사이인지)까지만.
3. secretTeaser는 200자 이상, 이 커플의 궁합에서 가장 결정적인 통찰 하나(가장 위험한 갈등 신호 / 가장 큰 강점 / 결혼·동거 적기 중 궁합 데이터상 가장 두드러지는 것 하나)를 골라 결제를 유도하는 톤으로 쓰되, 가장 핵심적인 단어·구간 2~4곳을 [BLUR]...[/BLUR] 태그로 감싸 숨겨라 (너무 많으면 누더기, 적으면 궁금증이 안 생긴다). 오행·십성·합충 용어 절대 금지. 작성 예시: "두 분의 궁합에서 가장 위험한 순간은 [BLUR]권태기가 시작되는 시점[/BLUR]입니다. 이때 ${myName}님은 [BLUR]무심하게 반응하는 방식[/BLUR]으로 상대를 더 멀어지게 만들 수 있어요.")
...
JSON 포맷:
{
  ...
  "essence": "궁합의 핵심 진단 (300~400자·정확히 2문단)",
  "secretTeaser": "결제를 유도하는 200자 이상 티저. [BLUR]...[/BLUR] 2~4개 포함"
}`;
};
