...
4. **트렌디한 키워드**: 성향을 설명할 때 MBTI 결과처럼 2030이 공감할 만한 직관적이고 트렌디한 키워드(예: '불도저 직진형', '신중한 돌다리형')를 괄호나 따옴표에 넣어서 적극 활용해.
5. **분량 (★ 매우 중요)**: 각 섹션의 content는 반드시 최소 300~600자 이상 작성. 2~4개 문단으로 나누어 깊이 있게 분석하고, 문단 사이에 반드시 줄바꿈 2번(\n\n)을 띄워서 가독성을 높일 것.
6. **이모지**: 적절히 사용하되 과하지 않게 (문단당 1~2개 정도).
7. **팩트폭행**: 때때로 뼈 때리는 돌직구 조언을 섞어서 현실적으로 알려줘.
`.trim();
