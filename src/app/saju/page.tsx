...
function Faq() {
  ...
  const faqs = [
    { q: "사주로 정말 재회 가능성을 알 수 있나요?", a: "두 사람의 만세력(사주 원국)에서 나타나는 합·충·형·해 관계와 오행 균형을 데이터로 계산해 재회 가능성을 점수화하고, 향후 6개월 중 두 사람의 기운이 가장 잘 맞물리는 시기(골든 윈도우)를 산출합니다. 같은 정보를 입력하면 항상 같은 결과가 나오는 정통 명리학 기반 연산이며, AI는 이 데이터를 심리·관계의 언어로 풀어 설명하는 역할만 담당합니다. 미래를 보장하는 예언이 아니라, 관계를 새로운 관점에서 이해하고 다가갈 시점을 정하는 참고 자료로 활용해 주세요." },
    ...
    { q: "상대방이 제가 분석한 걸 알 수 있나요?", a: "절대 알 수 없습니다. 분석은 입력하신 생년월일 정보만으로 이루어지며, 상대방에게 어떤 알림이나 연락도 가지 않습니다. 결과는 본인만 볼 수 있는 링크로 제공됩니다." },
    { q: "결과가 마음에 안 들면 환불이 되나요?", a: "무료 Lite 분석으로 먼저 결과의 방향을 확인하신 뒤 결제 여부를 결정하실 수 있습니다. 유료 리포트는 결제 즉시 맞춤 분석이 시작되는 디지털 콘텐츠 특성상, 분석이 개시된 후에는 단순 변심에 의한 환불이 어렵습니다(전자상거래법 제17조). 다만 시스템 오류로 리포트가 정상 제공되지 않은 경우에는 재제공 또는 전액 환불해 드립니다." },
