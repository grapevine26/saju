...
export default function HistoryDetailPage() {
    ...
    // 프리미엄 재열람 시 후기 작성 여부 — 아직 안 썼을 때만(false) 후기 폼 노출
    ...
    const startPremiumAnalysis = async (identifier: { type: 'guest' | 'member', value: string }, email: string, forcedPackageId?: string) => {
        ...
        try {
            ...
            const currentRecord = reunionHistory.find(r => r.id === id);
            ...
            localStorage.setItem('pendingTossPayment', JSON.stringify({
                ...
                customerEmail: email,
                recordId: id,
                metDate: (currentRecord as any)?.metDate || '',
            ...
            await payment.requestPayment({
                ...
                orderId: orderId,
                orderName: orderName,
                customerName: currentRecord?.myInfo?.name || '익명',
                customerEmail: email,
    ...
    // 후기 작성 여부 조회 (프리미엄 리포트 재열람 전용)
    useEffect(() => {
        const jobId = record?.premiumJobId;
        if (!record || record.tier !== 'premium' || !jobId) return;
        fetch(`/api/reviews?jobId=${jobId}`)
            .then(r => r.json())
            .then(d => { if (d.success) setReviewed(!!d.reviewed); })
            .catch(() => { /* 조회 실패 시 폼을 띄우지 않음 (reviewed=null 유지) */ });
    }, [record]);
    ...
    return (
        <div className="min-h-screen pb-24 relative" style={{background:'var(--bg-primary)'}}>
            ...
            <main className="p-6 pt-20 space-y-8">
                ...
                {/* ── 타로 브릿지 (Premium 전용) — 결과 페이지와 동일한 마무리 흐름 ── */}
                {!isLite && (
                    <motion.div
                ...
                {/* ── 후기 폼 — 아직 후기를 쓰지 않은 프리미엄 잡에만 노출 ── */}
                {!isLite && record.premiumJobId && reviewed === false && (
                    <ReviewForm jobId={record.premiumJobId} service="saju" />
                )}
