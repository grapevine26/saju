...
- \`advice\`는 최소 2~3문장 정도로 작성하며 300~400자 분량으로 작성해줘, 의미가 전환될 때 반드시 줄바꿈(\\n)을 사용하여 가독성을 높일 것.
...
3. \`goldenWindowMonths\`: 위 골든 윈도우 데이터 중 에너지 점수가 가장 높은 1개의 달(Month)을 선정하고, 해당 달 안에서 연락하기 좋은 날짜(goodDates) 2~4개와 절대 연락하면 안 되는 날짜(badDates) 2~4개를 배열(숫자)로 생성해줘. month는 "5월" 형태로.
